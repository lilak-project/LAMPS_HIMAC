void drawEvent()
{
    ...
    auto histXY = new TH2D("histXY","energy deposit (upstream view);X (mm);Y (mm)", 100,-100,100, 100,-100,100);
    histXY -> SetStats(0);
    auto cvsXY = cvsAll -> cd(1);
    cvsXY -> SetMargin(0.15,0.15,0.15,0.15);
    tree -> Draw("MCStepTPC.fY:MCStepTPC.fX>>histXY",cut,"colz");

    auto histZX = new TH2D("histZX","energy deposit (top view);Z (mm);X (mm)", 100,-80,80, 100,-100,100);
    histZX -> SetStats(0);
    ...
    auto histZY = new TH2D("histZY","energy deposit (side view);Z (mm);Y (mm)", 100,-80,80, 100,-100,100);
    histZY -> SetStats(0);
